...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) { // If everything's OK.
        ...
        //  Test for unique id:
        $q = "SELECT TEAM_ID FROM TEAM WHERE Team_Name='$name' AND ID != $id";
        $r = @mysqli_query($dbc, $q);
        if (mysqli_num_rows($r) == 0) {

            // Make the query:
            $q = "UPDATE TEAM SET Location_Name='$tn', Street='$street', City='$city', State='$state', ZIP='$zip', Team_Name='$name' WHERE ID=$id LIMIT 1";
            $r = @mysqli_query ($dbc, $q);
