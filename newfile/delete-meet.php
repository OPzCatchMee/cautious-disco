   ...
   <div class="rgba-background clearfix borderbox" id="page"><!-- group -->
    ...
    <div class="clearfix grpelem" id="ppu2370"><!-- column -->
     ...
     <div class="colelem shared_content" id="u37128" data-content-guid="u37128_content"><!-- custom html -->
      <?php
// This page is for deleting a meet (sets a flag but doesn't actually delete a meet).
echo '<h1>Delete a Meet</h1>';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST'  && $_SESSION['Is_Admin'] ) {
    if ($_POST['sure'] == 'Yes') { // Delete the record.
        // Make the query:
        $q = "UPDATE MEET SET Deleted = '1' WHERE ID=$id";
        $r = @mysqli_query ($dbc, $q);
        if (mysqli_affected_rows($dbc) == 1) { // If it ran OK.
            // Print a message:
            echo '<p>The meet has been deleted.</p>';
        } else { // If the query did not run OK.
            echo '<p class="error">The meet could not be deleted due to a system error.</p>'; // Public message.
            echo '<p>' . mysqli_error($dbc) . '<br />Query: ' . $q . '</p>'; // Debugging message.
        }
    } else { // No confirmation of deletion.
        echo '<p>The meet has NOT been deleted.</p>';
    }
} else if($_SESSION['Is_Admin']) { // Show the form.
    // Retrieve the user's information:
    $q = "SELECT Competition_Name FROM MEET WHERE ID=$id";
    $r = @mysqli_query ($dbc, $q);
